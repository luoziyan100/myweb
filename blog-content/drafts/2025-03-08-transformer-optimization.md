...
Transformer中的自注意力机制可以表述为对查询向量(Query)、键向量(Key)和值向量(Value)的操作。给定输入序列X，我们首先通过三个不同的变换矩阵W<sup>Q</sup>, W<sup>K</sup>, W<sup>V</sup>计算查询、键和值：
...
其中d<sub>k</sub>是键向量的维度，用于缩放以防止点积结果过大导致softmax梯度消失。
...
```
输出 = softmax(QK^T / √d_k) × V
```

## 3. 多头注意力机制

为了增强模型的表达能力，Transformer使用了多头注意力(Multi-Head Attention)机制。多头注意力并行运行多个自注意力"头"，每个头使用不同的投影矩阵W<sup>Q</sup>, W<sup>K</sup>, W<sup>V</sup>，允许模型同时关注不同的表示子空间：
